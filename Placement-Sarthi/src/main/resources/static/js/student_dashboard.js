...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (user.role !== 'student') {
        await showAlertModal('Access denied. Student privileges required.');
        window.location.href = 'login_page.html';
        return;
    }
...
async function registerForEvent(eventId) {
    ...
    if (!event) {
        await showAlertModal('Event not found');
        return;
    }

    if (!checkEligibility(event)) {
        await showAlertModal('You are not eligible for this event');
        return;
    }

    if (await showConfirmModal(`Register for ${event.organizingCompany} - ${event.jobRole}?`)) {
        try {
            const currentUser = JSON.parse(sessionStorage.getItem('currentUser'));
            ...
            if (response.ok) {
                await showAlertModal('Successfully registered!');
                await loadParticipations();
                showEventsForCurrentTab();
            } else {
                const errorData = await response.json();
                await showAlertModal(errorData.message || 'Registration failed');
            }
        } catch (error) {
            console.error('Error registering for event:', error);
            await showAlertModal('Network error. Please try again.');
        }
    }
}
...
async function viewEventDetails(eventId) {
    try {
        ...
        await showAlertModal('Error loading event details');
    }
}

async function viewParticipationDetails(eventId) {
...
async function showEventForm() {
    await showAlertModal('Please use the "Register" button on individual event cards to register for events.');
}
...
async function showMessage(message, type = 'info', duration = 5000) {
